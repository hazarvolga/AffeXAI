...
class TrackingService {
    endpoint = '/email-marketing/track';
    /**
     * Generate tracking URLs for email content
     */
    async generateTrackingUrls(campaignId, recipientEmail) {
        return http_client_1.httpClient.postWrapped(`${this.endpoint}/generate?campaignId=${campaignId}&recipientEmail=${encodeURIComponent(recipientEmail)}`);
    }
    /**
     * Get tracking statistics for a campaign
     */
    async getTrackingStats(campaignId) {
        return http_client_1.httpClient.getWrapped(`${this.endpoint}/stats/${campaignId}`);
    }
    /**
     * Process email content to add tracking
     */
    processEmailContent(htmlContent, trackingData) {
     ...
     */
    generateClickUrl(trackingId, originalUrl) {
        const baseUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:9006';
...
exports.default = exports.trackingService;
