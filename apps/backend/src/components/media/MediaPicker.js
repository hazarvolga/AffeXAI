...
function MediaPicker({ value, onChange, placeholder = "Medya seçin" }) {
            ...
            <button_1.Button variant="outline" className="w-full justify-start">
              {selectedMedia ? (<div className="flex items-center gap-2">
                  {selectedMedia?.type === 'image' && selectedMedia?.url ? (<div className="relative h-6 w-6 rounded overflow-hidden">
                      {selectedMedia.url.startsWith('/uploads/') ? (
                // For localhost uploads, use regular img tag
                <img src={`http://localhost:9005${selectedMedia.url}`} alt={selectedMedia.altText || selectedMedia.originalName} className="object-cover w-full h-full"/>) : (
                // For external images, use next/image
                  ...
                  {filteredMedia.map((media) => (<div key={media.id} className={`border rounded-lg overflow-hidden cursor-pointer transition-colors ${selectedMediaId === media.id
                    ...
                    : 'border-muted hover:border-primary/50'}`} onClick={() => handleSelect(media.id)}>
                      <div className="relative aspect-square">
                        {media.type === 'image' && media.url ? (media.url.startsWith('/uploads/') ? (
                // For localhost uploads, use regular img tag
                <img src={`http://localhost:9005${media.url}`} alt={media.altText || media.originalName} className="object-cover w-full h-full"/>) : (
                // For external images, use next/image
                <image_1.default src={`http://localhost:9005${media.url}`} alt={media.altText || media.originalName} fill className="object-cover"/>)) : (<div className="flex flex-col items-center justify-center h-full bg-muted">
                            <div className="mb-2">
