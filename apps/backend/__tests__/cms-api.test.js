...
const cms_service_1 = require("../src/lib/cms/cms-service");
describe('CMS API Integration Tests', () => {
    ...
    it('should create a new page', async () => {
        const page = await cms_service_1.cmsService.createPage(testPage);
    ...
    it('should create a new component', async () => {
        ...
        const component = await cms_service_1.cmsService.createComponent(componentData);
    ...
    it('should retrieve a page with components', async () => {
        const page = await cms_service_1.cmsService.getPage(createdPageId, true);
        expect(page).toHaveProperty('id');
        expect(page.title).toBe(testPage.title);
        expect(Array.isArray(page.components)).toBe(true);
    });
    it('should update a component', async () => {
        const updatedComponent = await cms_service_1.cmsService.updateComponent(createdComponentId, {
            props: {
    ...
    it('should reorder components', async () => {
        const reordered = await cms_service_1.cmsService.reorderComponents([createdComponentId], [5]);
        expect(reordered).toHaveLength(1);
        expect(reordered[0].orderIndex).toBe(5);
    });
    it('should create a page version', async () => {
        const version = await cms_service_1.cmsService.createPageVersion(createdPageId);
        expect(version).toHaveProperty('id');
        expect(version.version).toBe(2);
        expect(version.parentId).toBe(createdPageId);
    });
    it('should get page versions', async () => {
        const versions = await cms_service_1.cmsService.getPageVersions(createdPageId);
        expect(versions).toHaveLength(2);
    });
    it('should batch create components', async () => {
        const components = await cms_service_1.cmsService.batchCreateComponents([
            {
                pageId: createdPageId,
                type: 'button',
                props: {
    ...
    it('should publish a page', async () => {
        const publishedPage = await cms_service_1.cmsService.publishPage(createdPageId);
        expect(publishedPage.status).toBe('published');
        expect(publishedPage.publishedAt).toBeDefined();
    });
    it('should delete a component', async () => {
        await cms_service_1.cmsService.deleteComponent(createdComponentId);
        // Verify component is deleted by trying to fetch it
        try {
            await cms_service_1.cmsService.getComponent(createdComponentId);
        }
        catch (error) {
    ...
    it('should delete a page', async () => {
        await cms_service_1.cmsService.deletePage(createdPageId);
        // Verify page is deleted by trying to fetch it
        try {
            await cms_service_1.cmsService.getPage(createdPageId);
        }
        catch (error) {
            expect(error).toBeDefined();
        }
...
//# sourceMappingURL=cms-api.test.js.map