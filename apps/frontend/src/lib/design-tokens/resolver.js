...
exports.getAliasPath = getAliasPath;
exports.getTokenByPath = getTokenByPath;
exports.resolveToken = resolveToken;
exports.resolveAllTokens = resolveAllTokens;
exports.toCSSVariableName = toCSSVariableName;
exports.toCSSValue = toCSSValue;
exports.exportToCSS = exportToCSS;
exports.toSCSSVariableName = toSCSSVariableName;
exports.exportToSCSS = exportToSCSS;
...
exports.unflattenTokens = unflattenTokens;
exports.mergeTokens = mergeTokens;
exports.filterTokensByType = filterTokensByType;
// ============================================================================
 ...
 */
function resolveToken(value, tokens, visited = new Set()) {
 ...
 */
function toCSSVariableName(path, prefix) {
 ...
 */
function toCSSValue(token, tokens) {
    try {
        ...
        switch (token.$type) {
            ...
            case 'typography': {
                ...
                return `${fontWeight} ${fontSize}/${lineHeight} ${fontFamily}${letterSpacing ? ` ${letterSpacing}` : ''}`;
            }
            case 'shadow': {
                if (typeof value !== 'object')
                    return String(value);
                if (Array.isArray(value)) {
                    return value
                        .map((s) => `${s.offsetX} ${s.offsetY} ${s.blur}${s.spread ? ` ${s.spread}` : ''} ${s.color}`)
                        .join(', ');
                }
                return `${value.offsetX} ${value.offsetY} ${value.blur}${value.spread ? ` ${value.spread}` : ''} ${value.color}`;
            }
            case 'border':
                if (typeof value !== 'object')
                    return String(value);
            ...
            case 'transition':
                if (typeof value !== 'object')
                    return String(value);
                return `all ${value.duration}${value.delay ? ` ${value.delay}` : ''} ${Array.isArray(value.timingFunction)
 ...
 */
function exportToCSS(tokens, options = {}) {
 ...
 */
function toSCSSVariableName(path, prefix) {
 ...
 */
function exportToSCSS(tokens, options = {}) {
 ...
 */
function exportToJSON(tokens, metadata) {
    return {
        version: '1.0.0',
 ...
 */
function unflattenTokens(flattened) {
    const tokens = {};
 ...
 */
function mergeTokens(...tokenSets) {
    const merged = {};
