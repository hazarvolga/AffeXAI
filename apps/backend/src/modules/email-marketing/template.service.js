...
let TemplateService = (() => {
    ...
    var TemplateService = class {
        ...
        templatesRepository;
        templateFileService;
        constructor(templatesRepository, templateFileService) {
        ...
        async createFromExistingFile(fileTemplateName, customName) {
            try {
                const content = await this.templateFileService.getTemplateFileContent(`${fileTemplateName}.tsx`);
                const createDto = {
                    name: customName || this.formatTemplateName(fileTemplateName),
                    content,
                    type: email_template_entity_1.TemplateType.CUSTOM,
                    fileTemplateName,
                    isActive: true,
                };
                ...
                throw new common_1.NotFoundException(`Could not create template from file ${fileTemplateName}`);
            }
        }
