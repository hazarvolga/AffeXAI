...
async function seedTestData() {
    const dataSource = new typeorm_1.DataSource({
        ...
        entities: [learned_faq_entry_entity_1.LearnedFaqEntry],
        synchronize: false,
    });
    try {
        await dataSource.initialize();
        console.log('Database connected');
        const faqRepo = dataSource.getRepository(learned_faq_entry_entity_1.LearnedFaqEntry);
        ...
        console.log('\nBy Source:');
        console.log(`- Chat: ${testFaqs.filter(f => f.source === learned_faq_entry_entity_1.FaqEntrySource.CHAT).length} FAQs`);
        console.log(`- Tickets: ${testFaqs.filter(f => f.source === learned_faq_entry_entity_1.FaqEntrySource.TICKET).length} FAQs`);
        console.log(`- Suggestions: ${testFaqs.filter(f => f.source === learned_faq_entry_entity_1.FaqEntrySource.USER_SUGGESTION).length} FAQs`);
...
seedTestData();
//# sourceMappingURL=test-faq-dashboard.js.map