    ...
    var RolesService = class {
        ...
        async update(id, updateRoleDto) {
            ...
            if (role.isSystem && updateRoleDto.name && updateRoleDto.name !== role.name) {
                throw new common_1.BadRequestException('Cannot change name of system roles');
            }
        ...
        async remove(id) {
            ...
            if (role.users && role.users.length > 0) {
                throw new common_1.BadRequestException(`Cannot delete role "${role.displayName}" because it has ${role.users.length} assigned user(s)`);
            }
