...
export async function runCachingDemo() {
  ...
  const users1 = await usersService.getAll();
  ...
  const campaigns1 = await emailCampaignsService.getAll();
  ...
  const campaigns2 = await emailCampaignsService.getAll();
