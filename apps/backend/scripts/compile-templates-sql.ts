...
async function main() {
  ...
  // Get all templates with structure but no compiledHtml
  const result = await pool.query(
    `SELECT id, name, structure
     FROM email_templates
     WHERE structure IS NOT NULL
       AND ("compiledHtml" IS NULL OR "compiledHtml" = '')
     ORDER BY name`
  );

  const templates = result.rows;

  console.log(`\n📧 Found ${templates.length} templates to compile\n`);
  ...
  for (const template of templates) {
    try {
      // Compile structure → MJML → HTML (without header/footer - backend adds them dynamically)
      ...
      await pool.query(
        `UPDATE email_templates
         SET "compiledHtml" = $1, "compiledMjml" = $2
         WHERE id = $3`,
        [html, mjml, template.id]
      );
