...
class UrlValidator {
    ...
    static validate(url) {
        try {
            ...
            const hasBlockedExtension = this.BLOCKED_EXTENSIONS.some(ext => pathname.endsWith(ext));
     ...
     */
    static isSafeForCrawling(url) {
        const validation = this.validate(url);
        if (!validation.isValid) {
            return false;
        }
        // Additional safety checks
        const urlObj = new url_1.URL(validation.normalizedUrl);
