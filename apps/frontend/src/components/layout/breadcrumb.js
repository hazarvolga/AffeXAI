...
// ============================================================================
function Breadcrumb({ items: customItems, showHomeIcon = true, separator, variant = 'default', className, hiddenPaths = ['/'], }) {
    ...
    return (<nav aria-label="Breadcrumb" className={(0, utils_1.cn)('w-full py-3', variantStyles[variant], className)}>
      <div className="container mx-auto px-4">
        <ol className="flex items-center gap-2 text-sm">
          {items.map((item, index) => {
            const isFirst = index === 0;
            const isLast = item.isCurrentPage;
            return (<li key={item.href} className="flex items-center gap-2">
                {/* Separator (except before first item) */}
                {!isFirst && (<span className="text-muted-foreground/50" aria-hidden="true">
                    {separator || <lucide_react_1.ChevronRight className="h-4 w-4"/>}
                  </span>)}
                
                {/* Breadcrumb Item */}
                {isLast ? (<span className={(0, utils_1.cn)(currentPageStyles[variant])} aria-current="page">
                    {isFirst && showHomeIcon ? (<lucide_react_1.Home className="h-4 w-4"/>) : (item.label)}
                  </span>) : (<link_1.default href={item.href} className={(0, utils_1.cn)('transition-colors duration-200', linkStyles[variant])}>
                    {isFirst && showHomeIcon ? (<lucide_react_1.Home className="h-4 w-4"/>) : (item.label)}
                  </link_1.default>)}
              </li>);
        })}
