...
exports.logPerformanceSummary = logPerformanceSummary;
...
class MetricsStore {
    ...
    add(name, value) {
        const entry = {
            name,
            value,
            rating: (0, web_vitals_1.getMetricRating)(name, value),
            timestamp: Date.now(),
     ...
     */
    getSummary() {
        const metrics = ['FCP', 'LCP', 'FID', 'CLS', 'TTFB', 'INP'];
        const summary = {};
    ...
    checkBudgets() {
        ...
        Object.entries(summary).forEach(([name, stats]) => {
            if (stats) {
                const budget = web_vitals_1.PERFORMANCE_BUDGETS[name].good;
                results.push({
                    metric: name,
 ...
 */
exports.metricsStore = new MetricsStore();
// Load metrics on initialization
if (typeof window !== 'undefined') {
    exports.metricsStore.load();
}
/**
...
function getDashboardData() {
    return {
        summary: exports.metricsStore.getSummary(),
        budgetStatus: exports.metricsStore.checkBudgets(),
        recentMetrics: exports.metricsStore.getAllMetrics().slice(-20), // Last 20 metrics
...
function logPerformanceSummary() {
    const summary = exports.metricsStore.getSummary();
    const budgets = exports.metricsStore.checkBudgets();
    ...
    Object.entries(summary).forEach(([name, stats]) => {
        if (stats) {
            ...
            console.log(`  Average: ${(0, web_vitals_1.formatMetricValue)(name, stats.average)}`);
            console.log(`  Median: ${(0, web_vitals_1.formatMetricValue)(name, stats.median)}`);
            console.log(`  P95: ${(0, web_vitals_1.formatMetricValue)(name, stats.p95)}`);
...
function generatePerformanceReport() {
    ...
    Object.entries(dashboard.summary).forEach(([name, stats]) => {
        if (stats) {
            ...
            report += `- Average: ${(0, web_vitals_1.formatMetricValue)(name, stats.average)}\n`;
            report += `- P95: ${(0, web_vitals_1.formatMetricValue)(name, stats.p95)}\n`;
            report += `- Budget: ${(0, web_vitals_1.formatMetricValue)(name, budget?.budget || 0)}\n\n`;
        }
    });
