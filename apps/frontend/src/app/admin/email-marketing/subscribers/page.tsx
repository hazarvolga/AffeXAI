...
import CustomFieldsManager from '@/components/admin/email-marketing/custom-fields-manager';
...
export default function SubscribersManagementPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const subscribersData = await subscribersService.getAll();
      ...
      await Promise.all(
        selectedSubscribers.map(id => {
          ...
          if (subscriber) {
            ...
            return subscribersService.update(id, { groups: updatedGroups });
          }
  ...
  const handleUnsubscribe = async () => {
    try {
      ...
      await Promise.all(
        selectedSubscribers.map(id => 
          subscribersService.update(id, { status: 'unsubscribed' })
        )
      );
  ...
  const handleDelete = async () => {
    try {
      ...
      await Promise.all(
        selectedSubscribers.map(id => subscribersService.delete(id))
      );
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-wrap items-center justify-end gap-2">
        <Button variant="outline" asChild>
          <Link href="/admin/email-marketing/subscribers/import">
            <FileUp className="mr-2 h-4 w-4" />
            Toplu İçe Aktar
          </Link>
        </Button>
        <Button asChild>
          <Link href="/admin/email-marketing/subscribers/new">
                      ...
                      {sortedSubscribers.map((subscriber) => (
                        <TableRow key={subscriber.id}>
                          ...
                          <TableCell className="font-medium">
                            <Link 
                              href={`/admin/email-marketing/subscribers/${subscriber.id}/detail`} 
                    ...
                    <TableBody>
                      {segments.map((segment) => (
                        <TableRow key={segment.id}>
                          <TableCell className="font-medium">
                            <Link href={`/admin/email-marketing/segments/${segment.id}`} className="hover:underline">
                              {segment.name}
                            </Link>
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell>{typeof segment.openRate === 'number' ? segment.openRate.toFixed(2) : '0.00'}%</TableCell>
                          <TableCell>{typeof segment.clickRate === 'number' ? segment.clickRate.toFixed(2) : '0.00'}%</TableCell>
          ...
          <Card>
            <CardHeader>
              <div className="flex justify-between items-center">
                ...
                <Button asChild>
                  <Link href="/admin/email-marketing/groups">
                    <PlusCircle className="mr-2 h-4 w-4" />
                    Yeni Grup Oluştur
                  </Link>
                </Button>
                    ...
                    <TableBody>
                      {groups.map((group) => (
                        <TableRow key={group.id}>
                          <TableCell className="font-medium">
                            <Link href={`/admin/email-marketing/groups/${group.id}`} className="hover:underline">
                              {group.name}
                            </Link>
                          </TableCell>
      ...
      <Dialog open={isAddToGroupModalOpen} onOpenChange={setIsAddToGroupModalOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedSubscribers.length} abone seçildi. Bu aboneleri bir gruba ekleyin.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleAddToGroup} disabled={!addGroupName}>
              {selectedSubscribers.length} Aboneyi Gruba Ekle
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isRemoveFromGroupModalOpen} onOpenChange={setIsRemoveFromGroupModalOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedSubscribers.length} abone seçildi. Bu aboneleri bir gruptan çıkarın.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
          ...
          <DialogFooter>
            ...
            <Button 
              ...
              variant="destructive"
            >
              {selectedSubscribers.length} Aboneyi Gruptan Çıkar
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className="sm:max-w-[500px]">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleDelete} variant="destructive">
              {selectedSubscribers.length} Aboneyi Sil
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
