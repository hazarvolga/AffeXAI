...
exports.searchComponents = exports.getAllCategories = exports.getComponentById = exports.getComponentsByCategory = exports.componentsRegistry = exports.normalizeComponentId = void 0;
 ...
 */
const normalizeComponentId = (id) => {
    return legacyIdMap[id] || id;
};
exports.normalizeComponentId = normalizeComponentId;
/**
...
const getCategoryFromId = (id) => {
    ...
    if (id.startsWith('hero-'))
        return 'Hero';
    if (id.startsWith('content-') && !id.includes('variant'))
        return 'Content';
    ...
    if (id.startsWith('ecommerce-'))
        return 'Ecommerce';
    if (id.startsWith('blog-') || id.startsWith('rss-'))
        return 'Blog';
    ...
    if (id.startsWith('pricing-'))
        return 'Pricing';
    if (id.startsWith('rating-') || id.startsWith('review-'))
        return 'Rating';
 ...
 */
const generateDefaultProps = (config) => {
    const props = {};
 ...
 */
exports.componentsRegistry = Object.keys(block_configs_1.allBlockConfigs).map(id => ({
    id,
    ...
    defaultProps: generateDefaultProps(block_configs_1.allBlockConfigs[id]),
}));
/**
 * Get components by category
 */
const getComponentsByCategory = (category) => {
    return exports.componentsRegistry.filter(comp => comp.category === category);
};
...
const getComponentById = (id) => {
    ...
    const normalizedId = (0, exports.normalizeComponentId)(id);
    return exports.componentsRegistry.find(comp => comp.id === normalizedId);
};
...
const searchComponents = (query) => {
    ...
    return exports.componentsRegistry.filter(comp => comp.name.toLowerCase().includes(lowerQuery) ||
