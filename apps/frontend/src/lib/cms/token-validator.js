...
exports.validateTemplateTokens = validateTemplateTokens;
exports.validateBlockTokens = validateBlockTokens;
exports.validateTemplateConstraints = validateTemplateConstraints;
exports.validateTemplateContext = validateTemplateContext;
exports.getTokenUsageReport = getTokenUsageReport;
exports.checkTemplateSafety = checkTemplateSafety;
 ...
 */
function validateTemplateTokens(template, tokens) {
    const errors = [];
    const warnings = [];
    ...
    if (template.designSystem?.colorScheme) {
        for (const [key, tokenRef] of Object.entries(template.designSystem.colorScheme)) {
            if (typeof tokenRef === 'string' && (0, cms_template_1.isTokenAlias)(tokenRef)) {
                const path = (0, cms_template_1.extractTokenPath)(tokenRef);
                const value = (0, token_resolver_1.getTokenValue)(tokens, path);
                if (value === undefined) {
                    errors.push({
 ...
 */
function validateBlockTokens(block, tokens) {
    const errors = [];
    // Extract all token paths used in this block
    const usedTokens = (0, token_resolver_1.extractUsedTokens)(block.properties);
    // Validate each token exists
    for (const tokenPath of usedTokens) {
        const value = (0, token_resolver_1.getTokenValue)(tokens, tokenPath);
        if (value === undefined) {
            errors.push({
                type: 'token_not_found',
 ...
 */
function validateTemplateConstraints(template) {
    const errors = [];
    const warnings = [];
 ...
 * @returns Validation result
 */
function validateTemplateContext(template, context, mode, tokens) {
    const errors = [];
    const warnings = [];
 ...
 */
function countTokenReferences(template) {
    let count = 0;
    // Count design system tokens
    if (template.designSystem?.colorScheme) {
        count += Object.values(template.designSystem.colorScheme).filter(cms_template_1.isTokenAlias).length;
    }
    if (template.designSystem?.typography) {
        count += Object.values(template.designSystem.typography).filter(cms_template_1.isTokenAlias).length;
    }
    if (template.designSystem?.spacing) {
        count += Object.values(template.designSystem.spacing).filter(cms_template_1.isTokenAlias).length;
    }
    // Count block tokens
    for (const block of template.blocks) {
 ...
 */
function countBlockTokenReferences(block) {
    let count = (0, token_resolver_1.extractUsedTokens)(block.properties).length;
    if (block.children) {
        for (const child of block.children) {
...
function getTokenUsageReport(template) {
    const allTokens = new Set();
    const tokensByCategory = {};
    const blocksUsingTokens = [];
    // Collect from design system
    const collectTokensFromValue = (value) => {
        if (typeof value === 'string' && (0, cms_template_1.isTokenAlias)(value)) {
            const path = (0, cms_template_1.extractTokenPath)(value);
            allTokens.add(path);
    ...
    // Collect from blocks
    for (const block of template.blocks) {
        const blockTokens = (0, token_resolver_1.extractUsedTokens)(block.properties);
    ...
    return {
        ...
        tokensByCategory: Object.fromEntries(Object.entries(tokensByCategory).map(([k, v]) => [k, Array.from(v)])),
        blocksUsingTokens,
    };
}
function checkTemplateSafety(template, tokens) {
    ...
    if (missingTokens.length > 0) {
        recommendations.push(`Add missing tokens to your design system: ${missingTokens.join(', ')}`);
    }
