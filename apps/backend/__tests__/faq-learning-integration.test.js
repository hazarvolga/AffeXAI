...
const faq_learning_controller_1 = require("../src/modules/faq-learning/controllers/faq-learning.controller");
const faq_learning_service_1 = require("../src/modules/faq-learning/services/faq-learning.service");
const faq_learning_config_entity_1 = require("../src/modules/faq-learning/entities/faq-learning-config.entity");
const faq_ai_service_1 = require("../src/modules/faq-learning/services/faq-ai.service");
describe('FAQ Learning Settings Integration', () => {
    ...
    beforeEach(async () => {
        const module = await testing_1.Test.createTestingModule({
            controllers: [faq_learning_controller_1.FaqLearningController],
            providers: [
                faq_learning_service_1.FaqLearningService,
                {
                    provide: (0, typeorm_1.getRepositoryToken)(faq_learning_config_entity_1.FaqLearningConfig),
                    useValue: mockConfigRepository,
                },
                {
                    provide: faq_ai_service_1.FaqAiService,
        ...
        await app.init();
        configRepository = module.get((0, typeorm_1.getRepositoryToken)(faq_learning_config_entity_1.FaqLearningConfig));
        faqLearningService = module.get(faq_learning_service_1.FaqLearningService);
        faqAiService = module.get(faq_ai_service_1.FaqAiService);
    });
    afterEach(async () => {
        await app.close();
        jest.clearAllMocks();
    ...
    describe('Configuration API Integration', () => {
        describe('GET /faq-learning/config', () => {
            ...
            it('should include proper validation metadata for each setting', async () => {
                ...
                const minConfidenceConfig = configs.find((c) => c.key === 'minConfidenceForReview');
        ...
        describe('PUT /faq-learning/config/bulk', () => {
            ...
            it('should validate configuration values and return errors for invalid data', async () => {
                ...
                expect(response.body.results).toBeDefined();
                const failedResults = response.body.results.filter((r) => !r.success);
        ...
        describe('POST /faq-learning/config/reset/:sectionKey', () => {
            it('should reset configuration section to defaults', async () => {
                ...
                response.body.resetConfigs.forEach((config) => {
                    expect(config).toHaveProperty('key');
    ...
    describe('Category-wise Reset Functionality', () => {
        it('should reset only specified category without affecting others', async () => {
            ...
            const thresholdConfigs = initialConfigs.filter((c) => c.category === 'thresholds');
            const processingConfigs = initialConfigs.filter((c) => c.category === 'processing');
            ...
            const afterThresholdConfigs = afterResetConfigs.filter((c) => c.category === 'thresholds');
            const afterProcessingConfigs = afterResetConfigs.filter((c) => c.category === 'processing');
            // Thresholds should be reset to defaults
            afterThresholdConfigs.forEach((config) => {
                const defaultConfig = faq_learning_config_entity_1.FaqLearningConfig.getDefaultConfig();
                expect(config.value).toBe(defaultConfig[config.key]);
            });
            // Processing configs should remain unchanged
            afterProcessingConfigs.forEach((config, index) => {
    ...
    describe('Error Handling and Edge Cases', () => {
        it('should handle network connectivity issues gracefully', async () => {
            // Mock network error
            jest.spyOn(faqAiService, 'getProviderStatus').mockRejectedValueOnce(new Error('Network error'));
            const response = await request(app.getHttpServer())
                .get('/faq-learning/provider-status')
                .expect(500);
    ...
    describe('System Integration', () => {
        it('should maintain consistency between config API and service layer', async () => {
            ...
            const pipelineConfig = await faqLearningService.getPipelineConfig();
...
//# sourceMappingURL=faq-learning-integration.test.js.map