...
let PredictiveAnalyticsService = (() => {
    ...
    var PredictiveAnalyticsService = class {
        ...
        static {
            ...
            PredictiveAnalyticsService = _classThis = _classDescriptor.value;
        ...
        constructor(campaignRepository, emailLogRepository, subscriberRepository, openHistoryRepository) {
         ...
         */
        async predictCampaignPerformance(campaignId, subscriberIds) {
            ...
            const engagementScores = await Promise.all(subscriberIds.slice(0, 100).map(id => this.calculateEngagementScore(id)));
            ...
            // Performans skoru
            const performanceScore = Math.round((expectedOpenRate * 40 + expectedClickRate * 40 + expectedConversionRate * 20) * 100);
            // Güven skoru
         ...
         */
        async calculateChurnRisk(subscriberId) {
         ...
         */
        async generateAIInsights(campaignId) {
            const insights = [];
            ...
            if (recentCampaigns.length >= 3) {
                // Açılma oranı trendi
                const openRates = recentCampaigns.map(c => c.sentCount > 0 ? (c.openedCount || 0) / c.sentCount : 0);
        ...
        async getDashboardMetrics() {
            ...
            const engagementScores = await Promise.all(sampleSubscribers.map(s => this.calculateEngagementScore(s.id)));
            const averageEngagementScore = Math.round(engagementScores.reduce((acc, s) => acc + s.score, 0) / engagementScores.length);
    ...
    return PredictiveAnalyticsService = _classThis;
})();
