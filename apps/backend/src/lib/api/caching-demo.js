...
async function runCachingDemo() {
    ...
    const users1 = await usersService_1.default.getAllUsers();
    const endTime1 = performance.now();
    console.log(`   First request: ${Math.round(endTime1 - startTime1)}ms (${users1.length} users)`);
    // Second request should be faster (cached)
    const startTime2 = performance.now();
    const users2 = await usersService_1.default.getAllUsers();
    ...
    const events1 = await eventsService_1.default.getAllEvents();
    const endTime3 = performance.now();
    console.log(`   First request: ${Math.round(endTime3 - startTime3)}ms (${events1.length} events)`);
    // Second request should be faster (cached)
    const startTime4 = performance.now();
    const events2 = await eventsService_1.default.getAllEvents();
    ...
    const campaigns1 = await emailCampaignsService_1.default.getAllCampaigns();
    ...
    const campaigns2 = await emailCampaignsService_1.default.getAllCampaigns();
